...
if [[ ${memdump_file_val} != "0" ]]; then
	...
	start=`date +%s`
	if [ -n "$(moredump.bin -help | grep -- -bmm)" ]; then
		moredump.bin ${moredumpdir}/moredump_${DATE_TAG}.bmm -bmm -xml_path ${xml_dir} -firmware_binary ${base_dir}/ 2>>${status_file} >/dev/null
		moredump_ext=bmm
	else
	...
	script_status=$?
	echo "Generated moredump moredump_${DATE_TAG}.${moredump_ext}|moredump_${DATE_TAG}.err.${moredump_ext}" >>${status_file}
	end=`date +%s`
	echo "moredump generated in ${moredumpdir} in $((end-start)) seconds" >> ${status_file} 2>&1
	if [ -f ${moredumpdir}/moredump_${DATE_TAG}.${moredump_ext} ]; then
		chmod 755 ${moredumpdir}/moredump_${DATE_TAG}.${moredump_ext}
		cp -a ${moredumpdir}/moredump_${DATE_TAG}.${moredump_ext} ${logdir}
		if [ $? -eq 0 ]; then
			echo "copied ${moredumpdir}/moredump_${DATE_TAG}.${moredump_ext} to ${logdir}" >> ${status_file} 2>&1
		else
			echo "copy failed($?) ${moredumpdir}/moredump_${DATE_TAG}.${moredump_ext} is not copied." >> ${status_file} 2>&1
		fi
	elif [ -f ${moredumpdir}/moredump_${DATE_TAG}.err.${moredump_ext} ]; then
		chmod 755 ${moredumpdir}/moredump_${DATE_TAG}.err.${moredump.ext}
		cp -a ${moredumpdir}/moredump_${DATE_TAG}.err.${moredump_ext} ${logdir}
		if [ $? -eq 0 ]; then
			echo "copied ${moredumpdir}/moredump_${DATE_TAG}.err.${moredump_ext} to ${logdir}" >> ${status_file} 2>&1
		else
			echo "copy failed($?) ${moredumpdir}/moredump_${DATE_TAG}.err.${moredump_ext} is not copied." >> ${status_file} 2>&1
		fi
	else
		echo "Cannot find moredump_${DATE_TAG}.${moredump_ext} or moredump_${DATE_TAG}.err.${moredump_ext} in ${moredumpdir}" >> ${status_file}
	fi
	sync
fi
...
kernel_log="KERNELLOG.tmp"
if [ ${sable} -eq 1 ]; then
	logcat_dmesg_logs

	# if kernel log extracted from reserved memory, replace captured one by dmesg from it
	if [ -f ${dir}/${kernel_log} ]; then
		cp ${dir}/${kernel_log} ${logdir}/kernel_${DATE_TAG}.log 2>&1
		echo "copied to ${logdir}/kernel_${DATE_TAG}.log" >> ${status_file} 2>&1

		cp ${dir}/${kernel_log} ${common_logdir}/kernel_${DATE_TAG}.log 2>&1
		echo "copied to ${common_logdir}/kernel_${DATE_TAG}.log" >> ${status_file} 2>&1

		rm ${dir}/${kernel_log}
		#rm ${logdir}/dmesg_${DATE_TAG}.log 2>>${status_file}
		#echo "removed ${logdir}/dmesg_${DATE_TAG}.log" >> ${status_file} 2>&1
	fi
else
...
if [ -f ${logdir}/dmesg_${DATE_TAG}.log ]; then
